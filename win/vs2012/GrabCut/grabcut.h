...
GrabCut::GrabCut()
{
	...
	std::cout << "default mode: foreground. " << std::endl;
}	

int GrabCut::run(Mat img, Mat &msk)
{
	_src	= img;
	...
	while(1)
	{
		...
		{			
			...
			_mode = GC_FGD;		// forground mode
			std::cout << "switched to foreground mode" << std::endl;
		}else if(c=='b'){
			_mode = GC_BGD;		// background mode
			std::cout << "switch to background mode" << std::endl;
		}else if(c=='r')		// reset
		{
